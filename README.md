...
기본 학습은 기존과 동일하게 window 단위 stratified random split을 사용합니다. 새로운 recording 세션에 대한 일반화 성능을 확인하려면 session-wise split을 사용할 수 있습니다.

```powershell
.\venv\Scripts\python.exe train.py --dataset-dir new_dataset --split-mode session --filter-mode highpass_20 --window-size 200 --pre-event 80 --model-output best_emg_model_session.pt
```

`--split-mode session`은 CSV recording 파일 단위로 train/val/test를 분리하므로, 같은 세션에서 추출된 window가 서로 다른 split에 섞이지 않습니다. 이 결과는 random split보다 낮게 나올 수 있지만, 실제 새로운 세션에서의 성능을 더 보수적으로 확인하는 데 적합합니다.
...
GUI 키보드 범례는 다음 의미를 가집니다.

- `Emitted`: 최종 입력으로 확정된 키
- `Top candidate`: 모델이 현재 가능성이 높다고 판단한 후보 키
- `Pending`: 후보로 감지됐지만 아직 vote 조건 등을 기다리는 키
- `Skipped`: confidence 또는 margin 조건을 만족하지 못해 무시된 키
- `Calibration`: active calibration 중 감지된 후보 키
